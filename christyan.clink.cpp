/*
Este é meu código do compactador/descompactador. Há algumas mudanças a serem implementadas, como:
* - Manejar algumas propriedades dos objetos;
* - Criação de objetos intermediários para reduzir a complexidade dos objetos maiores;
* - Mudar a limitação da compressão de 4 bytes para ilimitado;
* - Criação do objeto "limpa neve" para que o descompactador seja capaz de ler o arquivo compactado sem perda ou corrupção de dados;
*/

/* # ENGLISH
This is my compressor/decompressor code. There are some changes to be implemented, such as:
* - Handling some properties of the objects;
* - Creating intermediate objects to reduce the complexity of larger objects;
* - Changing the compression limit from 4 bytes to unlimited;
* - Creating the "snow cleaner" object so that the decompressor can read the compressed file without data loss or corruption;
*/
